...
- 本 SPEC 系列 review 簽核(校正/演算法兩方;rev 2026-07-17:專案不進產線,產線簽核角色移除)。
- 開放問題清單(§7)全數關閉或明確延期。
...
1. 真實 sensor 的 PD offsets 與「均勻子格」假設差異多大?
   —— **不在 scope / 外部承擔(2026-07-17 關閉)**:主要影響外部 SAD 模組,
   本工具僅 config 數值連動;介面品質由 §3a 誤差預算把關。
   (原「延期至 M2 前」;專案不碰硬體後不再是本工具待辦。)
2. runtime PDLIB 實際 disparity 單位?
   —— **不在 scope / 保留 config 選項(2026-07-17 關閉)**:不做實體燒錄,
   `output_disparity_unit` 雙路徑皆實作並由 UT-09 覆蓋、report 雙單位記錄;
   若外部燒錄端有定義,翻 config 值即可。(原「延期至 M2 實體燒錄前」。)
